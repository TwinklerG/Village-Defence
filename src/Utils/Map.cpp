...
std::vector<TowerInfo> Map::LoadTowerInfo() {
  ...
  for (const auto &d : l_towerData) {
    l_towerInfos.push_back({d["tag"], d["cost"], d["atk"], d["range"],
...
void Map::Save() {
  ...
  for (const auto &l_fig : m_figures) {
    nlohmann::json l_figState = {
        ...
        {"m_position",
         nlohmann::json::array({l_fig->GetSprite().getPosition().x,
    ...
    m_labels.emplace_back(
        ...
        m_fonts["arial"], [i, this]() {
          ...
          if (nlohmann::json data = nlohmann::json::parse(in);
            ...
            out << std::setw(4) << data;
            out.close();
            m_labels[i].SetTextString(
                std::to_string(static_cast<int>(data["props"][i]["stock"])));
...
Map::Map(sf::RenderWindow *l_wind, const nlohmann::json &l_gameState)
    : m_isWin(false),
      ...
      m_atomResolution({l_gameState["m_atomResolution"][0],
