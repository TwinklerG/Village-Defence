...
Map::Map(sf::RenderWindow *l_wind, const int l_level, std::string l_resolutions,
         const sf::Vector2f l_resolution)
  : m_isWin(false), m_resolution(std::move(l_resolutions)),
    m_atomResolution(l_resolution), m_level(l_level) {
  OnCreate(l_wind);
}
...
void Map::OnCreate(sf::RenderWindow *l_wind) {
  ...
  m_board = std::make_unique<Board>(
    sf::Vector2f(m_atomResolution.x, m_atomResolution.y),
    l_wind->getSize().y / 35);
  m_wind = l_wind;
...
void Map::Update(const sf::RenderWindow *l_wind, const sf::Time &l_time) {
  ...
  if (std::accumulate(m_startPoints.begin(), m_startPoints.end(), 0,
                      [](const int accum, const StartPoint &l_sp) {
      ...
      for (int j = 0; j < m_YRange; j++) {
        if (gl::checkMouseSelect(*m_places[i][j], m_wind)) {
          if (m_selected.m_tower &&
              ...
              (m_places[i][j]->GetPlaceType() == PlaceType::Land ||
               m_places[i][j]->GetPlaceType() == PlaceType::Tower)) {
            // Lay a Tower
            ...
            m_places[i][j]->GetTower()->SetCircle(
              std::make_shared<sf::CircleShape>(l_circle));
          }
        }
      }
    }
      ...
      if (m_places[i][j]->GetPlaceType() == PlaceType::Tower) {
        if (m_places[i][j]->GetTower()->GetCalmTime().asSeconds() > 0) {
          m_places[i][j]->GetTower()->SetCalmTime(
            m_places[i][j]->GetTower()->GetCalmTime() - l_time);
          continue;
        }
        for (const auto &l_fig: m_figures) {
          if (gl::checkInRange(
            *l_fig, *m_places[i][j],
            static_cast<float>(m_places[i][j]->GetTower()->GetRange()) *
            m_atomResolution.x)) {
            int cnt = 0;
            ...
            m_bullets.emplace_back(
              cs, l_fig,
              static_cast<int>(m_places[i][j]->GetTower()->GetBulletSpeed()),
              m_places[i][j]->GetTower()->GetAttackPoint());
            break;
          }
        }
        m_places[i][j]->GetTower()->SetCalmTime(sf::seconds(
          m_places[i][j]->GetTower()->m_totalCalmTime.asSeconds() /
          static_cast<float>(m_propBuff.m_accelerate)));
      }
    }
  }
  // Bullets Update
  ...
  for (auto l_b: m_bullets) {
    ...
    const double dx = -l_b.GetCircle().getPosition().x +
                      l_b.GetTargetFigure()->getPosition().x;
    const double dy = -l_b.GetCircle().getPosition().y +
                      l_b.GetTargetFigure()->getPosition().y;
    const double d = sqrt(dx * dx + dy * dy);
    l_b.GetCircle().setPosition(
      {
        static_cast<float>(l_b.GetCircle().getPosition().x +
                           l_b.GetSpeed() / d * l_time.asSeconds() * dx),
        static_cast<float>(l_b.GetCircle().getPosition().y +
                           l_b.GetSpeed() / d * l_time.asSeconds() * dy)
      });
  ...
  for (const auto &l_fig: m_figures) {
    for (const auto &l_ep: m_endPoints) {
      if (gl::checkInRange(*l_fig, l_ep,
                           l_fig->getSize().x / 2.0 + l_ep.getSize().x / 2.0)) {
        --m_lives;
        m_textBox->AddText("Village Invaded, current lives: " +
                           std::to_string(m_lives));
        if (auto itr =
              std::find(next_figures.begin(), next_figures.end(), l_fig);
          itr != next_figures.end()) {
          next_figures.erase(itr);
        }
        break;
      }
      if (std::find(next_figures.begin(), next_figures.end(), l_fig) ==
          next_figures.end()) {
        next_figures.emplace_back(l_fig);
      }
    }
  ...
  for (const auto &[l_choice, l_towerInfo]: m_choices) {
    if (gl::checkMouseSelectSprite(*l_choice.first, m_wind) &&
      ...
      m_selected.m_tower = std::make_shared<Tower>(
        *l_choice.first, l_choice.first->getTexture().getSize(),
        sf::seconds(static_cast<float>(l_towerInfo.m_calmTime)),
        l_towerInfo.m_attackPoint, static_cast<float>(l_towerInfo.m_speed),
        l_towerInfo.m_range, l_towerInfo.m_cost, l_towerInfo.m_red,
        l_towerInfo.m_green, l_towerInfo.m_blue, l_towerInfo.m_bulletRadius,
        l_towerInfo.m_tag);
...
void Map::LoadMap() {
  ...
  m_choices = std::vector<std::pair<
    std::pair<std::shared_ptr<sf::Sprite>, std::shared_ptr<sf::Text> >,
  ...
  for (int i = 0; i < l_choiceSum; ++i) {
    ...
    if (m_textures.find("tower" + std::to_string(tag)) == m_textures.end()) {
      m_textures["tower" + std::to_string(tag)] =
          sf::Texture("res/img/tower/" + m_resolution + "/tower" +
                      std::to_string(tag) + ".png");
    }
    m_choices[i].first.first =
        std::make_shared<sf::Sprite>(m_textures["tower" + std::to_string(tag)]);
    m_choices[i].first.first->setOrigin(
      {
        static_cast<float>(m_choices[i].first.first->getTexture().getSize().x /
                           2.0),
        static_cast<float>(m_choices[i].first.first->getTexture().getSize().y /
                           2.0)
      });
    m_choices[i].first.first->setPosition(
      {
        static_cast<float>(m_choices[i].first.first->getTexture().getSize().x /
                           2.0 +
                           m_atomResolution.x * static_cast<float>(i + 1)),
        static_cast<float>(m_choices[i].first.first->getTexture().getSize().y /
                           2.0)
      });
    if (m_fonts.find("arial") == m_fonts.end()) {
    ...
    m_choices[i].first.second = std::make_unique<sf::Text>(
      m_fonts["arial"], std::to_string(l_towerInfos[tag].m_cost),
    ...
    m_choices[i].first.second->setPosition(
      m_choices[i].first.first->getPosition() +
      sf::Vector2f(
        0, static_cast<float>(
          m_choices[i].first.first->getTexture().getSize().y / 2.0)));
  ...
  for (int j = 0; j < m_YRange; ++j) {
    for (int i = 0; i < m_XRange; ++i) {
      if (const int l_placeType = l_placesType[j][i]; l_placeType == 0) {
        if (m_textures.find("grass0") == m_textures.end()) {
          m_textures["grass0"] =
              sf::Texture("res/img/grass/" + m_resolution + "/grass0.png");
        }
        ...
        m_places[i][j] = std::make_shared<Place>(
          l_sp, m_textures["grass0"].getSize(), PlaceType::Land);
      } else if (l_placeType == 1) {
        if (m_textures.find("road") == m_textures.end()) {
          m_textures["road"] =
              sf::Texture("res/img/grass/" + m_resolution + "/road.png");
        }
        ...
        m_places[i][j] = std::make_shared<Place>(
          l_sp, m_textures["road"].getSize(), PlaceType::Road);
      } else if (l_placeType == 3) {
        if (m_textures.find("startPoint") == m_textures.end()) {
          m_textures["startPoint"] =
              sf::Texture("res/img/grass/" + m_resolution + "/startPoint.png");
        }
        ...
        m_places[i][j] = std::make_shared<Place>(
          l_sp, m_textures["startPoint"].getSize(), PlaceType::Road);
        m_startPoints.emplace_back(l_sp, m_textures["startPoint"].getSize(),
                                   std::make_pair(i, j), l_invaderTurns,
                                   m_resolution, m_atomResolution);
      } else if (l_placeType == 4) {
        if (m_textures.find("endPoint") == m_textures.end()) {
          m_textures["endPoint"] =
              sf::Texture("res/img/grass/" + m_resolution + "/endPoint.png");
        }
        ...
        if (m_textures.find("grass2") == m_textures.end()) {
          m_textures["grass2"] =
              sf::Texture("res/img/grass/" + m_resolution + "/grass2.png");
        }
        ...
        m_places[i][j] = std::make_shared<Place>(
          l_sp, m_textures["grass2"].getSize(), PlaceType::End);
        m_endPoints.emplace_back(l_esp, l_esp.getTexture().getSize(),
                                 std::make_pair(i, j));
      }
    }
  }
    ...
    while (!q.empty()) {
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        if (tx < 0 || ty < 0 || tx >= m_XRange || ty >= m_YRange ||
            ...
            (m_places[tx][ty]->GetPlaceType() != PlaceType::Road &&
             m_places[tx][ty]->GetPlaceType() != PlaceType::End)) {
          continue;
        }
  ...
  std::transform(m_roads.begin(), m_roads.end(), std::back_inserter(l_roads),
...
std::vector<TowerInfo> Map::LoadTowerInfo() {
  ...
  for (const auto &d: l_towerData) {
    l_towerInfos.push_back({
      d["tag"], d["cost"], d["atk"], d["range"],
      d["calmTime"], d["speed"], d["rgb"][0], d["rgb"][1],
      d["rgb"][2], d["bulletRadius"]
    });
  }
  sort(l_towerInfos.begin(), l_towerInfos.end(),
       [](const TowerInfo &l_towerInfo1, const TowerInfo &l_towerInfo2) {
...
void Map::Save() {
  ...
  l_gameState["m_atomResolution"] =
      nlohmann::json::array({m_atomResolution.x, m_atomResolution.y});
  ...
  for (int j = 0; j < m_YRange; ++j) {
    for (int i = 0; i < m_XRange; ++i) {
      ...
      if (m_places[i][j]->GetPlaceType() == PlaceType::Tower) {
        l_places[i][j].second = {
          m_places[i][j]->GetTower()->GetCalmTime().asSeconds(),
          m_places[i][j]->GetTower()->GetTag()
        };
      } else {
  ...
  for (const auto &l_fig: m_figures) {
    nlohmann::json l_figState = {
      ...
      {"m_reward", l_fig->GetReward()},
      {
        "m_position",
        nlohmann::json::array({
          l_fig->GetSprite().getPosition().x,
          l_fig->GetSprite().getPosition().y
        })
      },
    };
    ...
    nlohmann::json l_bulletState = {
      {
        "position", nlohmann::json::array(
          {l_bullet.getPosition().x, l_bullet.getPosition().y})
      },
  ...
  for (const auto &l_startPoint: m_startPoints) {
    ...
    l_startPointState["m_calmTime"] = l_startPoint.GetCalmTime().asSeconds();
...
void Map::LoadProp() {
  ...
  if (m_textures.find("ACCELERATE") == m_textures.end()) {
    m_textures["ACCELERATE"] =
        sf::Texture("res/img/prop/" + m_resolution + "/prop_accelerate.png");
  }
  if (m_textures.find("DECELERATE") == m_textures.end()) {
    m_textures["DECELERATE"] =
        sf::Texture("res/img/prop/" + m_resolution + "/prop_decelerate.png");
  }
  for (int i = 0; i < l_props.size(); ++i) {
    std::string l_propName = l_props[i]["name"],
        l_propCnt = nlohmann::to_string(l_props[i]["stock"]);
    m_labels.emplace_back(
      m_textures[l_propName],
      sf::Vector2f(
        m_atomResolution.x * static_cast<float>(6 + i) +
        static_cast<float>(m_textures[l_propName].getSize().x) / 2.0f,
        static_cast<float>(m_textures[l_propName].getSize().y) / 2.0f),
      l_propCnt, static_cast<unsigned int>(m_atomResolution.x) / 3,
      m_fonts["arial"], [i, this]() {
        std::ifstream in("res/config/store.json");
        if (nlohmann::json data = nlohmann::json::parse(in);
          data["props"][i]["stock"] > 0) {
          in.close();
          data["props"][i]["stock"] =
              static_cast<int>(data["props"][i]["stock"]) - 1;
...
Map::Map(sf::RenderWindow *l_wind, const nlohmann::json &l_gameState)
  : m_isWin(false),
    ...
    m_atomResolution({
      l_gameState["m_atomResolution"][0],
      l_gameState["m_atomResolution"][1]
    }),
  ...
  for (int i = 0; i < l_startPointsInfo.size(); ++i) {
    ...
    m_startPoints[i].SetCalmTime(sf::seconds(l_startPointsInfo[i]["m_calmTime"]));
  }
  LoadProp();
  ...
  // m_places = std::vector<std::vector<Place> >(m_XRange,
  // std::vector<Place>(m_YRange));
  for (int j = 0; j < m_YRange; ++j) {
    for (int i = 0; i < m_XRange; ++i) {
      if (l_places[i][j][0] == static_cast<int>(PlaceType::Tower)) {
        ...
        if (m_textures.find("tower" + std::to_string(l_tag)) ==
          ...
          m_textures["tower" + std::to_string(l_tag)] =
              sf::Texture("res/img/tower/" + m_resolution + "/tower" +
                          std::to_string(l_tag) + ".png");
        }
        const auto [l_choice, l_towerInfo] = m_choices[l_tag];
        m_places[i][j]->SetPlaceType(PlaceType::Tower);
        m_places[i][j]->SetTower(std::make_shared<Tower>(
          *l_choice.first, l_choice.first->getTexture().getSize(),
          sf::seconds(static_cast<float>(l_towerInfo.m_calmTime)),
          l_towerInfo.m_attackPoint, static_cast<float>(l_towerInfo.m_speed),
          l_towerInfo.m_range, l_towerInfo.m_cost, l_towerInfo.m_red,
        ...
        m_places[i][j]->GetTower()->GetSprite().setOrigin(
          {
            static_cast<float>(
              m_textures["tower" + std::to_string(l_tag)].getSize().x) /
            2.0f,
            static_cast<float>(
              m_textures["tower" + std::to_string(l_tag)].getSize().y) /
            2.0f
          });
  ...
  for (const auto &l_fig: l_figures) {
    ...
    std::transform(l_fig["m_increments"].begin(), l_fig["m_increments"].end(),
                   std::back_inserter(l_increments),
                   [](const int &l_increment) {
                     return static_cast<Direction>(l_increment);
                   });
    const std::string l_figureName =
        "invader" + std::to_string(static_cast<int>(l_fig["m_tag"]));
    if (m_textures.find(l_figureName) == m_textures.end()) {
      m_textures[l_figureName] = sf::Texture("res/img/invader/" + m_resolution +
                                             "/" + l_figureName + ".png");
    }
    ...
    m_figures.push_back(std::make_shared<Figure>(
      l_fig["m_tag"], l_sp, l_sp.getTexture().getSize(), l_increments,
  ...
  for (const auto &l_bullet: l_bullets) {
    const sf::Vector2f l_pos =
        sf::Vector2f(l_bullet["position"][0], l_bullet["position"][1]);
  ...
  m_textBox = std::make_unique<gl::TextBox>(
    ...
    sf::Vector2f(static_cast<float>(l_wind->getSize().x) / 5.0f * 2,
                 static_cast<float>(l_wind->getSize().y) / 5.0f),
    6, m_fonts["arial"], l_gameState["m_textBoxMessages"]);
