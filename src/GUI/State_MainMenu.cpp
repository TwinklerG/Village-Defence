...
void State_MainMenu::OnCreate() {
  ...
  m_text.setPosition(static_cast<float>(m_stateMgr->GetContext()->m_wind->GetWindowSize().x) / 2.0f,
                     static_cast<float>(m_stateMgr->GetContext()->m_wind->GetWindowSize().y) / 5.0f * 2.0f);
  ...
  m_buttonPos = sf::Vector2f(static_cast<float>(m_stateMgr->GetContext()->m_wind->GetWindowSize().x) / 2.0f,
                             static_cast<float>(m_stateMgr->GetContext()->m_wind->GetWindowSize().y) / 20.0f * 11.0f);
  ...
  for (int i = 0; i < m_ButtonSum; ++i) {
    sf::Vector2f buttonPosition(m_buttonPos.x, m_buttonPos.y + static_cast<float>(i) * (m_buttonSize.y + static_cast<float>(m_buttonPadding)));
...
void State_MainMenu::MouseClick(EventDetails *l_details) {
  ...
  for (int i = 0; i < m_ButtonSum; ++i) {
    if (static_cast<float>(mousePos.x) >= m_rects[i].getPosition().x - halfX && static_cast<float>(mousePos.x) <= m_rects[i].getPosition().x + halfX && static_cast<float>(mousePos
        .y) >= m_rects[i].getPosition().y - halfY && static_cast<float>(mousePos.y) <= m_rects[i].getPosition().y + halfY) {
      if (i == 0) {
