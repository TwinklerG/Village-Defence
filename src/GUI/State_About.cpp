...
void State_About::OnCreate() {
  ...
  m_content = std::make_unique<sf::Text>(
    ...
    "现在，你可以敲击Escape进入主菜单",
    static_cast<unsigned int>(m_stateMgr->GetContext()->m_atomResolution.x) /
    3 * 2);
  m_content->setFillColor(sf::Color::White);
  m_content->setOrigin({
    m_content->getLocalBounds().size.x / 2.0f,
    m_content->getLocalBounds().size.y / 2.0f
  });
...
void State_About::Update(const sf::Time &l_time) {
  m_content->setPosition(
    {
      m_content->getPosition().x,
      m_content->getPosition().y - l_time.asSeconds() * 80
    });
