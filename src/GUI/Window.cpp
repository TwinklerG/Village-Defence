...
void Window::Setup(const std::string &l_title, const sf::Vector2u &l_size) {
  ...
  m_eventManager.AddCallback(static_cast<StateType>(0), "Fullscreen_toggle", &Window::ToggleFullscreen, this);
  m_eventManager.AddCallback(static_cast<StateType>(0), "Window_close", &Window::Close, this);

  Create();
}

void Window::Create() {
  const auto style = (m_isFullscreen ? sf::Style::Fullscreen : sf::Style::Close);
